...
//Incase of order display the Item table ..................................................................//
$('#to_interprise').change(function () { 
    if(Boolean(Number($('#savetranfer').data('order')))){
           let intp = $(this).find('option:selected').data('value')
    const pr = x => x.st === intp
    let itms = StoreNje.data.filter(pr)

    if(itms.length>0){
        $('#table_show').show(600)
        Items.state = itms
        coloredItem.state = OutColor.state,
        ItemsSize.state = OutSize.state
          ...
          //if the item include size place the size input.............................//
           ...
           sized.forEach(sz=>{
               if( sz.sized__color == colored[l].color && sz.bidhaa == colored[l].bidhaa){
                ...
                if(Number(colored[l].bidhaa__bidhaa__idadi_jum)>1 && Number(sz.idadi)/Number(colored[l].bidhaa__bidhaa__idadi_jum)>=1 ){
                    coloredone+=`
                   <div class="row py-1">
                         <div class="col-4">
                        ...
                        if(there_is){
                          coloredone+=` value=${parseInt(qty_set/Number(colored[l].bidhaa__bidhaa__idadi_jum))} `
                        }
                   ...
                   if(prs.length>0){
                       prs.forEach(pr=>{
                           if(Number(sz.idadi)/Number(pr.idadi)>=1){
                               coloredone+=`
                        <div class="row py-1">
                         <div class="col-4">
                        ...
                        //show the sat value if the user has alread sat the quantities for given size.....//
                        if(there_is){
                          coloredone+=` value=${parseInt(qty_set/Number(pr.qty))} `
                        }
            ...
            // IN CASE THE ITEM IS TO BE SOLD/PURCHASED IN WHOLESALE AND RETAIL include the wholesale input.................................//
                     if(parseInt(colored[l].bidhaa__bidhaa__idadi_jum)>1 && Number(colored[l].idadi)/Number(colored[l].bidhaa__bidhaa__idadi_jum)>=1 ){
                            coloredone+=`  
                                        <div class="row mb-2">
                                            <div class="col-4">
                                      ...
                                      coloredone+=`  
                                        <div class="row mb-2">
                                            ...
                                            <div class="col-8">
                                                ...
                                                if(there_is){
                                                    coloredone+=` value=${parseInt(qty_set/Number(pr.idadi))}`
                                                  }
                                            coloredone+=` >
...
function tablerow(pos){
    ...
    <tr id="list_tr${pos}" data-pos="${pos}">
      <td class="p-1 align-top trf-td-img">
      ...
      <td class="align-top trf-td-item">
        <div class="suggest-holder item-attr${pos}">
          ...
          <ul class="masaki masaki${pos}" data-hiarch="${pos}" style="z-index:999999;max-height:250px;overflow-y:scroll"></ul>
        </div>
      ...
      <td class="p-1 align-top trf-td-qty" data-label="${lang('Idadi','Qty')}">
      ...
      <td class="align-top text-center trf-td-rm">
...
//CONvvERT TRANSFER ORDER TO TRANSFERED ITEMS...........................................///
$('#hakiki_oda1').unbind('submit').submit(function (e) {
    e.preventDefault() 

    let url = $(this).attr('action'),
    csrfToken =   $('input[name=csrfmiddlewaretoken]').val(),

        trans = $(this).data('transfer') , 
        data= {
            data:{
...
$('#savetranfer').unbind('submit').submit(function (e) {
    ...
    let url=$(this).attr("action"),
      //  edit=$(this).data("edit"),
       // bill=$(this).data("bill"),
        oda = Number($(this).data('order')),
        csrfToken =   $('input[name=csrfmiddlewaretoken]').val(),
        code =  $('#transfer_code').val(),
        date = $('#transfer_date').val(),
        reason = $('#sababu_').val(),
        to = $('#to_interprise').find('option:selected').data('value'),itm_dt=[],zer=0,kuzid=0,
        allColored = [],
        allSized = []
...
const chosen_c = allColored.filter(c=>c.color===el),
    ...
    // console.log({sum:chosen_c.reduce((a,b)=> a + b.idadi,0),all:})
});
...
const chosen_i = itm_dt.filter(i=>i.val===el),
     all_i = Items.state.find(i=>i.id===el)
      
     if(Number(chosen_i.reduce((a,b)=> a + (b.idadi*b.uwiano),0))>(Number(all_i.idadi))){
         itm_exceed = true
     }
   // console.log({sum:chosen_i.reduce((a,b)=> a + (b.idadi*b.uwiano),0),all:all_i.idadi})
});
...
//comfirm receives....................................................................//
$('#hakiki_oda').click(function(){
    csrfToken =   $('input[name=csrfmiddlewaretoken]').val(),
    url = $(this).attr('action'),
    itm = $(this).data('transfer')
    ...
    success: function (respo) {
        ...
        if(respo.success){
            ...
            if(!Boolean(data.data.oda)){
             location.replace(`/stoku/viewTransfer?item_valued=${respo.bil}&backto=allbill`)
            }else{
             location.replace(`/stoku/viewReceives?item_valued=${respo.bil}&backto=allbill`)
...
//REMOVE ORDERD ITEM...........................................///
$('#toa_oda1').unbind('submit').submit(function (e) {
    e.preventDefault() 
...
//REMOVE ORDERD ITEM...........................................///
$('#toa_oda_new').unbind('submit').submit(function (e) {
    e.preventDefault() 
