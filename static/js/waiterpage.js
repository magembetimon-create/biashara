...
let WAITER_CART = []
let WAITER_ITEM_IMG = []
let WAITER_GROUPED_MEMBERS_MAP = {}
...
function waiterHandleBarcodeScan(code) {
  ...
  POSTREQUEST({
    ...
    // Merge looked-up row into local list when possible.
...
function renderWaiterTableGrid() {
  ...
  const areaHtml = WAITER_TABLE_AREAS.map(ar => `
    <button
      type="button"
      class="btn btn-sm waiter-area-btn text-left w-100 mb-1 ${Number(ar.id) === Number(WAITER_SELECTED_AREA_ID) ? 'btn-primary' : 'btn-light'}"
      data-id="${ar.id}">
  ...
  host.html(`
    <div class="row no-gutters">
      <div class="col-4 pr-2" style="max-height:220px; overflow-y:auto;">
        ${areaHtml}
      </div>
      <div class="col-8 pl-2">
        <div class="waiter-table-grid" style="max-height:220px; overflow-y:auto;">
          ${tablePaneHtml}
        </div>
      </div>
    </div>
...
function openWaiterPrintWindow(orderId) {
  const printUrl = `/mauzo/waiter_Invoprint?item_valued=${Number(orderId)}&lang=${Number(waiterLang(0,1))}`
...
function loadWaiterCatalog() {
  ...
  function postPage(offset, limit, first) {
    ...
    const data = {
      data: typeof waiterDeviceRequest === 'function' ? waiterDeviceRequest(payload) : payload,
...
$('body').on('click', '#waiterConfirmPrintedBtn', function() {
