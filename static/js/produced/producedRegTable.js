...
let idadi_item=0,zilizopo=0,thamani=0,mauzo_tegemeo=0,num=0
    ...
    if(data.products.length>0){ 
        ...
        const all_item = allItm.filter(x=>x.produced_id),
               itemsK = [...new Set(all_item.map(i=>i.bidhaa_id))]
               let items = itemsK.map(i=>getAllItm(i))

               function getAllItm(i){
                const ftf = itm=>itm.bidhaa_id === i
                   return {
                       id : i,
                       namba: all_item.filter(ftf)[0].namba,
                       jina: all_item.filter(ftf)[0].bidhaaN,
                       aina: all_item.filter(ftf)[0].ainaN,
                       aina_id: all_item.filter(ftf)[0].aina,
                       kampuni: all_item.filter(ftf)[0].brand,
                       kampuni_id: all_item.filter(ftf)[0].kampuni,
                       vipimo: all_item.filter(ftf)[0].vipimo,
                       vipimo_jum: all_item.filter(ftf)[0].vipimoJum,
                       idadi_jum: all_item.filter(ftf)[0].uwiano,
                       thamani: all_item.filter(ftf).reduce((a,b)=> a + ((b.Bei_kununua/b.uwiano)*b.idadi),0),
                       thamaniM: all_item.filter(ftf).reduce((a,b)=> a + (b.Bei_kuuza*b.idadi),0),
                       zote: all_item.filter(ftf).length,
                       idadi: all_item.filter(ftf).reduce((a,b)=> a +Number( b.idadi),0),
        ...
        let produced = prdxnCost.state.filter(i=>i.bidhaa===itm.id),qty=0,prW=0,prC = 0,Coast = itm.thamani
        if(produced.length>0) {
            prC =  Number(produced[0].cost) * Number(itm.idadi)
            Coast = Number(produced[0].cost) * Number(itm.idadi),
            qty = Number(produced.reduce((a,b)=> a + (b.qty),0))
            prW = Number(produced.reduce((a,b)=> a + ((b.cost)*b.qty),0))
        }
        ...
        trp+=`   </td> 
            ...
            <td>${itm.zote}</td>
            <td><a  href="/production/producedReg?f=${itm.aina_id}"> ${itm.aina.replace(/[&\/\\#,+()$~%"*?<>{}`]/g, "")}</a></td>
            <td><a  href="/production/producedReg?bf=${itm.kampuni_id}"> ${itm.kampuni.replace(/[&\/\\#,+()$~%"*?<>{}`]/g, "")}</a></td>`
            if(Number(Number(itm.idadi)/Number(itm.idadi_jum))>1){

                trp+=`  <td><span class="smallerFont text-primary">${itm.vipimo_jum} </span></td>
                        <td class="brown weight600">${Number(Number(prW).toFixed(2)).toLocaleString()}</td>
                        <td class=" weight600">${Number(Coast).toFixed(2)}</td>
                        <td> <span class="font-weight-bold">${Number(Number(qty)/Number(itm.idadi_jum)).toFixed(2)}</span> </td> 
                        <td> <span class="font-weight-bold">${Number(Number(itm.idadi)/Number(itm.idadi_jum)).toFixed(2)}</span> </td> 
        ...
        trp+=` 
            <td >
            <div class="d-flex">
                <a href="/production/producedItems?f=${itm.id}" class="btn btn-default  btn-sm text-primary " > 
