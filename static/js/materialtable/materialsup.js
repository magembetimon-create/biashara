 ...
 let idadi_item=0,zilizopo=0,thamani=0,mauzo_tegemeo=0,num=0
     if(sambazia.state.length>0){ 
         const itemsK = sambazia.state.map(i=>i.id),
                all_item = data.products.filter(x=>x.material)
         ...
         num+=1
             ...
             trp+=` </td>
                <td class="font-weight-bold" > <a class="btn btn-default text-primary" href="/production/MaterialReg?sup=${itm.id}" > ${itm.zote}</a></td>
                <td> <span class="font-weight-bold">${Number(itm.idadi).toFixed(2)}</span> </td>
                <td class="maroon">${Number(Number(itm.thamani).toFixed(2)).toLocaleString()}</td>
