...
if(data.products.length>0){ 
    let all_item = data.products.filter(x=>x.material)
    ...
    trp+=`   </td> 
        ...
        <td> <a href="/production/MaterialItems?f=${itm.bidhaa_id}"> ${itm.bidhaaN.replace(/[&\/\\#,+()$~%"*?<>{}`]/g, "")}</a></td>
        <td> <a  href="/production/MaterialReg?f=${itm.aina}"> ${itm.ainaN.replace(/[&\/\\#,+()$~%"*?<>{}`]/g, "")}</a></td>
        <td> <a href="/production/MaterialReg?bf=${itm.kampuni}"> ${itm.brand.replace(/[&\/\\#,+()$~%"*?<>{}`]/g, "")}</a></td>`
        if(Number(Number(itm.idadi)/Number(itm.uwiano))>1){

            trp+=` <td class="brown weight600">${Number(Number(Coast*itm.uwiano).toFixed(2)).toLocaleString()}</td>
                    <td><span class="smallFont text-primary">${itm.vipimoJum} </span></td>
                    <td> <span class="font-weight-bold">${Number(Number(itm.idadi)/Number(itm.uwiano)).toFixed(2)}</span> </td>` 
        }else{
            trp+=`

                    <td class="brown weight600">${Number(Number(Coast).toFixed(2)).toLocaleString()}</td>            
        ...
        if(itm.msambaji_id){
            trp+=` <a href="/stoku/MaterialItems?sup=${(itm.msambaji_id)}" class="latoFont">${(itm.vendor || 'None' ).replace(/[&\/\\#,+()$~%="*?<>{}`]/g, "") }</a> `
           }else{
