...
const csItemCell = (row) => {
  ...
  if (row.kind === 'line' && row.invo_id) {
    return `<a class="text-primary" href="/purchase/viewbill?item_valued=${row.invo_id}&backto=vendor">${name}</a>`;
  }
  return name;
};
...
window.csPrintStatement = () => {
  ...
  const title = lang('Taarifa ya Msambazaji', 'Vendor Statement');
  ...
  w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${title} - ${meta.jina || ''}</title>
    ...
    <h2>${title}</h2>
    <div class="meta">
      <div><strong>${lang('Msambazaji', 'Vendor')}:</strong> ${meta.jina || ''}</div>
