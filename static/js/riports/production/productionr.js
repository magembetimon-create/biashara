    ...
    if(theData.state.length>0){        
        ...
        let Df = i => !i.online,  Lf = i=>i.online,Na = ByU(),Tawi=branch(),Wvat = withVat()
           ...
           n+=1 
            ...
            let tha = dtP.filter(h=>h.uhamisho_id===null).reduce((a,b)=> a + (Number(b.Bei_kununua)*(b.bqty)/b.uwiano),0),
  ...
  const  dt =  theData.state.filter(i=>i.id===val)
       ...
       if(dt.length>0){
            const  theDT = dt[0],
                ...
                //VENDORS SELECTOR.................//
                vendor = [...new Set(theDT.dtP.filter(br).map(vn=>vn.Na))].map(v=> `<option class="text-capitalize" value="${v}" >${theDT.dtP.filter(vn=>vn.Na===v)[0].first_name} ${theDT.dtP.filter(vn=>vn.Na===v)[0].last_name} </option>`) 
                ...
                $('#toT').val(moment(theDT.To).endOf('day').format())                    
                         ...
                         duraTitle =  theDT.txt  
                        let trnd  = lang('Mwenendo wa Bidhaa zilizozalishwa','The Trend of Produced Goods')
...
function ItemsTrends(data){
      ...
      const {Purchases,transfer,adjst,Sales} = data,
        ...
        tha = {
                this:Purchases.this.filter(h=>h.uhamisho_id===null).reduce((a,b)=> a + (Number(b.Bei_kununua)*(b.bqty)/b.uwiano),0),
    ...
    //    paid = pay.reduce((a,b)=> a + Number(b.ilolipwa),0),
        ...
        vat={
            ...
            let matawi=$('#Matawini').data('branches')
                   tr = `
                   <tr>
                        <td colspan="${Number(matawi)==0?3:4}" class="brown weight600 manunuzi pt-3" >${lang('Uzalishaji','Production')}</td>
...
let LoC = Number($('#riportChatRist .btn-secondary').data('riport')),
    ...
    amounts =  dates.map((d)=>{
        let ft=t=>moment(t.date).format('YYYY-MM-DD') == d,
            pui =  ipu.filter(ft),
            nnt = pui.filter(h=>h.uhamisho_id===null).reduce((a,b)=> a + (Number(b.Bei_kununua)*(b.bqty)/b.uwiano),0),
...
if(LoC){
           ...
           //Draw new Chart................................................................................................................./
           let title = `<h6 class="py-2">${lang(`Takwimu za mwenendo wa bidhaa zilizozalishwa kwa kila `,`The Trend for Produced Goods Statistics on each `)} ${days<=31?lang('Siku','Day'):lang('Mwezi','Month')}, ${duraTitle}<h6/>`
...
let title = `<h6 class="py-2" >${lang(`Orodha ya Uzalishaji kwa kila `,`Production List on each `)} ${days<=31?lang('Siku','Day'):lang('Mwezi','Month')}, ${duraTitle}<h6/>`
...
const pui = itms.Purchases.this,
    ...
    function theItms(b){
        ...
        return {
            ...
            tha:itm.filter(h=>h.uhamisho_id===null).reduce((a,b)=> a + (Number(b.Bei_kununua)*(b.bqty)/b.uwiano),0),
            idadi:itm.filter(h=>h.uhamisho_id===null).reduce((a,b)=> a + Number((b.bqty)),0),
            bqty:itm.filter(h=>h.uhamisho_id===null).reduce((a,b)=> a + Number((b.bqty)),0),
    ...
    if(LoC){
        ...
        //Draw new Chart................................................................................................................./
        let title = `<h6 class="py-2 " >${lang(`Takwimu ya Uzalishaji na Mwenendo  kwa kila Bidhaa `,`Items Production and Trends Statistics on each Item  `)}, ${duraTitle}<h6/>`
         $('#theDataPanel').html(`${title} <div class="table-responsive" ><canvas style="${window.outerWidth>800?'max-height:85vh !important':'max-height:70vh !important'} "  id="myChartC"></canvas></div>`);
        ...
        <tr class="smallFont latoFont">
            ...
            <tr class="smallFont latoFont">
                <th class="manunuzi" colspan=2 >${lang('Zilizozalishwa','Produced')}</th>
...
`   
let title = `<h6 class="py-2" >${lang(`Orodha ya Uzalishaji na mwenendo kwa kila Bidhaa `,`Production and Trends List, on each Item Produced`)} , ${duraTitle}<h6/>`
$('#theDataPanel').html(title+td)
$('#SoldItems').DataTable();
    }
...
let aina = Categs.state,
    ...
    if(LoC){
        ...
        //Draw new Chart................................................................................................................./
        let title = `<h6 class="py-2 " >${lang(`Takwimu ya Uzalishaji namwenendo wa kila Aina ya Bidhaa `,`Production and Trends Statistics on each Item Category  `)}, ${duraTitle}<h6/>`
        ...
        var data= {
            labels: bidhaa.map(b=>b.name),
            datasets: [{
                label:lang("Zilizozolishwa","Produced"),
                fill: true,
                lineTension: 0,
                backgroundColor: "#1a8cff",
                borderColor: "#1a8cff", // The main line color
...
`   
let title = `<h6 class="py-2 " >${lang(`Orodha ya Uzalishaji na Mwenendo wa kila Aina ya Bidhaa `,`Production and Trend List on each Item Category  `)}, ${duraTitle}<h6/>`
            $('#theDataPanel').html(title+td)
            $('#SoldItems').DataTable();
    }
...
if(LoC){
    ...
    //Draw new Chart................................................................................................................./
    let title = `<h6 class="py-2 " >${lang(`Takwimu ya Uzalishaji na mwenendo wa kila Kundi la Bidhaa `,`Items Production and Trends Statistics on each Item Category Group `)}, ${duraTitle}<h6/>`
    $('#theDataPanel').html(`${title} <div class="table-responsive" ><canvas style="${window.outerWidth>800?'max-height:85vh !important':'max-height:75vh !important'} "  id="myChartC"></canvas></div>`);
    ...
    var data= {
        labels: bidhaa.map(b=>b.name),
        datasets: [{
            label:lang("Zilizozalishwa","Produced"),
            fill: true,
            lineTension: 0,
            backgroundColor: "#1a8cff",
            borderColor: "#1a8cff", // The main line color
...
`   
let title = `<h6 class="py-2 " >${lang(`Orodha ya Uzalishaji na Mwenendo wa kila Kundi la Bidhaa `,`Production and Trend List on each Item Category Group  `)}, ${duraTitle}<h6/>`
        $('#theDataPanel').html(title+td)
        $('#SoldItems').DataTable();
}
...
if(dt.length>0){
 ...
 let     tr = `<table id="table-Invoices" class="table table-bordered smallFont" style="width:100%">
            ...
            let d1= moment(muda.from).format('YYYY-MM-DD'),
                ...
                title = `<h6 class="py-2 " >${lang(`Orodha ya Toleo za Uzalishaji ${theDT.mark?' Zilizowekwa alama':''}`,`${theDT.mark?'Marked':''} Production Batches List on`)},<span class="darkblue"> ${dur}</span><h6/>`
   ...
   matawi = tw.map(t=>formT(t))
                ...
                <tr class="smallFont text-left" >
                    ...
                    <tbody class="cursor-pointer p-3" >
                                ...
                                <tr  class="cursor-pointer manunuzi weight600 ">    
                                    <td class="">${lang('Zilizozalishwa','Produced')}</td>
                                    <td ></td>
    ...
    if(LoC){
        ...
        var data= {
                ...
                datasets: [{
                    label:lang("Uzalishaji","Production"),
                    fill: true,
                    lineTension: 0,
                    backgroundColor: "#1a8cff",
                    borderColor: "#1a8cff", // The main line color
...
$('#SoldItems').DataTable();
    ...
    placedataPanel.prepend(`<h6 id="itmTitle" class="py-3" data-itm=${id} data-val=${val} data-num=1 >${lang('Uzalishaji na Mwenendo kwa','Production and trends for')} <i class="darkblue text-capitalize">${itms[0].bidhaaN}</i>  ${lang('Kwa kila Tawi','In each branch')}</h6> ${summary()}`)
...
,
     ...
     showbill = `<div class="d-block ">
                ...
                title = `<h6 class="py-2 " >${lang(`Onesha Toleo za Uzalishaji ${MARKED?' Zilizowekwa Alama':''}`,`View ${MARKED?' Marked':''} Production Batches on`)},<span class="darkblue"> ${dur}</span><h6/>`
             ...
             li+=`<li class="px-2 py-1 border-bottom  ${Number(b.id)==Number(bil_id)?'activeList':''} "  >
                    ...
                    <a class="row px-0 smallerFont text-decoration-none  viewBill2" data-val=${b.id} data-from="${muda.from}" data-to="${muda.to}" style="color: rgb(51, 51, 51);" type="button" >
                        ...
                        <div class="col-7 mx-0 latoFont  pt-0">
                            <span class="text-primary btn btn-default btn-sm smallerFont px-0 latoFont">BATCH-${b.code}</span> | ${moment(b.tarehe).format('DD/MM/YYYY')}
                        </div>
                 ...
                 <tr class="bg-light smallerFont ">
                    ...
                    if(puSize.length>0){
                     ...
                     <tr class="smallerFont latoFont" >
                        <th colspan=2 class="manunuzi" > ${lang('Zilizozalishwa','Produced')}</th>
                   ...
                   `
                    ...
                    summary += `
                                ...
                                <tr  class="cursor-pointer manunuzi">    
                                    <td class="">${lang('Thamani ya Toleo Jumla','Total Batch Worth')}</td>
                                    <td ></td>
...
const mark = Number($(this).data('mark')),
    ...
    $('#markTite').text(mark?lang('Ondoa Alama kwenye Uzalishaji','unMark Batch'):lang('Wekea  Alama Uzalishaji','Mark Batch'))
    $('#SubmitMark').text(mark?lang('Ondoa Alama','unMark Batch'):lang('Weka  Alama','Mark Batch'))
