const AddType = ()=> Number($('#AddType').val()) || 0
  ...
  const  dt =  theData.state.filter(i=>i.id===val)
       ...
       if(dt.length>0){
            const  theDT = dt[0],
                ...
                Df = i => !i.online,  Lf = i=>i.online,Tawi =branch(),
                //  BRANCH SELECTOR FILTER
                br  = d=>Number(d.duka)===Tawi && Number(d.shop)===Tawi && Number, nbr=d=>Number(d.shop)!=Tawi && Number(d.duka)===Tawi,Reg=AddType(),
                all = r =>r.reg || r.add,reg = r =>r.reg,add =r =>r.add,
                ByVn = Reg==0?all:Reg==1?reg:add,   
                ...
                $('#toT').val(moment(theDT.To).endOf('day').format())                    
                         ...
                         duraTitle =  theDT.txt   
                         let trnd  = lang('Mwenendo wa Bidhaa Zilizoongezwa','The Trend of Added Items')
...
function ItemsTrends(data){
    ...
    //    paid = pay.reduce((a,b)=> a + Number(b.ilolipwa),0),
        ...
        Ongeza ={ 
            this:adjst.this.filter(pr=>pr.Ongeza &&!(pr.add || pr.reg)).reduce((a,b)=> a + (b.tha*(b.qty)/b.uwiano),0),
            other:adjst.other.filter(pr=>pr.Ongeza &&!(pr.add || pr.reg)).reduce((a,b)=> a + (b.tha*(b.qty)/b.uwiano),0),
        ...
        vat={
            ...
            let matawi=$('#Matawini').data('branches')
                   tr = `
                   <tr>
                        <td colspan="${Number(matawi)==0?3:4}" class="brown weight600 manunuzi pb-2 pt-3" >${lang('Kuongezwa','Adding')}</td>
...
const LoC = Number($('#riportChatRist .btn-secondary').data('riport')),
    ...
    dates = [...new Set(ipu.map(i=>moment(i.date || i.tarehe).format('YYYY-MM-DD')))].sort(),
    months = [...new Set(dates.map(i=>moment(i).format('YYYYMM')))],

    amounts =  dates.map((d)=>{
        let ft=t=>moment(t.date || t.tarehe).format('YYYY-MM-DD') == d,
            pui =  ipu.filter(ft),
    ...
    mAmount =  months.map((d)=>{
        let ft=t=> Number(moment(t.date || t.tarehe).format('YYYYMM')) == Number(d),
            pui =  ipu.filter(ft),
            nnt =  pui.reduce((a,b)=> a + (Number(b.Bei_kununua)*(b.bqty)/b.uwiano),0),
            adj = Adjst.filter(ft),
            itms = aitms.filter(ft),
...
if(LoC){
           ...
           //Draw new Chart................................................................................................................./
           let title = `<h6 class="py-2" >${lang(`Takwimu za mwenendo wa bidhaa Zilizoongezwa kwa kila `,`The Trend for Added Items Statistics on each `)} ${days<=31?lang('Siku','Day'):lang('Mwezi','Month')}, ${duraTitle}<h6/>`
...
let title = `<h6 class="py-2" >${lang(`Orodha ya Kuongeza Bidhaa kwa kila `,`Items Adding List on each `)} ${days<=31?lang('Siku','Day'):lang('Mwezi','Month')}, ${duraTitle}<h6/>`
...
const pui = itms.Purchases.this,
    ...
    function theItms(b){
        ...
        return {
            ...
            added:adj.filter(pr=>pr.Ongeza&&!(pr.add || pr.reg)).reduce((a,b)=> a + Number((b.qty)),0),
    ...
    if(LoC){
        ...
        //Draw new Chart................................................................................................................./
        let title = `<h6 class="py-2 " >${lang(`Takwimu ya  Bidhaa Zilizoongezwa na Mwenendo  kwa kila Bidhaa `,`Items Adding and Trends Statistics on each Item  `)}, ${duraTitle}<h6/>`
         $('#theDataPanel').html(`${title} <div class="table-responsive" ><canvas style="${window.outerWidth>800?'max-height:85vh !important':'max-height:70vh !important'} "  id="myChartC"></canvas></div>`);
        ...
        <tr class="smallFont latoFont">
            ...
            <tr class="smallFont latoFont">
                <th class="manunuzi" colspan=2 >${lang('Zilizoongezwa','Added')}</th>
...
`   
let title = `<h6 class="py-2" >${lang(`Orodha ya Kuongeza na mwenendo kwa kila Bidhaa `,`Adding and Trends List, on each Item Added`)} , ${duraTitle}<h6/>`
$('#theDataPanel').html(title+td)
$('#SoldItems').DataTable();
    }
...
let aina = Categs.state,
    ...
    if(LoC){
        ...
        //Draw new Chart................................................................................................................./
        let title = `<h6 class="py-2 " >${lang(`Takwimu ya Bidhaa Zilizoongezwa namwenendo wa kila Aina ya Bidhaa `,`Items Adding and Trends Statistics on each Item Category  `)}, ${duraTitle}<h6/>`
        ...
        var data= {
            labels: bidhaa.map(b=>b.name),
            datasets: [{
                label:lang("Zilizoongezwa","Added Items"),
                fill: true,
                lineTension: 0,
                backgroundColor: "#1a8cff",
                borderColor: "#1a8cff", // The main line color
...
`   
let title = `<h6 class="py-2 " >${lang(`Orodha ya Bidhaa Zilizoongezwa na Mwenendo wa kila Aina ya Bidhaa `,`Added Items and Trend List on each Item Category  `)}, ${duraTitle}<h6/>`
            $('#theDataPanel').html(title+td)
            $('#SoldItems').DataTable();
    }
...
if(LoC){
    ...
    //Draw new Chart................................................................................................................./
    let title = `<h6 class="py-2 " >${lang(`Takwimu ya Bidhaa Zilizoongezwa na mwenendo wa kila Kundi la Bidhaa `,`Item Adding and Trends Statistics on each Item Category Group `)}, ${duraTitle}<h6/>`
    $('#theDataPanel').html(`${title} <div class="table-responsive" ><canvas style="${window.outerWidth>800?'max-height:85vh !important':'max-height:75vh !important'} "  id="myChartC"></canvas></div>`);
...
`   
let title = `<h6 class="py-2 " >${lang(`Orodha ya Zilizoongezwa na Mwenendo wa kila Kundi la Bidhaa `,`Added Items and Trend List on each Item Category Group  `)}, ${duraTitle}<h6/>`
        $('#theDataPanel').html(title+td)
        $('#SoldItems').DataTable();
}
...
if(dt.length>0){
     const ftt = i=>  moment(i.tarehe).startOf('day').format() >= moment(muda.from).startOf('day').format()&& moment(i.tarehe).endOf('day').format() <= moment(muda.to).endOf('day').format() ,
      ...
      br  = d=>Number(d.duka)===Tawi && Number(d.shop)===Tawi, nbr=d=>Number(d.shop)!=Tawi && Number(d.duka)===Tawi,Reg=AddType(),
      all = r =>r.reg || r.add,reg = r =>r.reg,add =r =>r.add,
      ByVn = Reg==0?all:Reg==1?reg:add,  
    ...
    return {
       ...
       tarehe:itm.filter(i=>i.bili===b)[0]?.tarehe,
       date:itm.filter(i=>i.bili===b)[0]?.date,

       user:itm.filter(i=>i.bili===b)[0]?.first_name + ' '+ itm.filter(i=>i.bili===b)[0]?.last_name,
 ...
 let     tr = `<table id="table-Invoices" class="table table-bordered smallFont" style="width:100%">
            ...
            billsii.forEach(a=>{
                ...
                tr+= `<tr> 
                  <td>${n}</td>
                  <td>ADJ-${a.code}</td>
                  <td>${moment(a.date || a.tarehe).format('DD-MM-YYYY')}</td>
            ...
            let d1= moment(muda.from).format('YYYY-MM-DD'),
                ...
                title = `<h6 class="py-2 " >${lang(`Orodha ya notisi za Kuongeza Bidhaa ${theDT.mark?' Zilizowekwa alama':''}`,`${theDT.mark?'Marked':''} Items adding List on`)},<span class="darkblue"> ${dur}</span><h6/>`
                ...
                <tr class="smallFont text-left" >
                    ...
                    <tbody class="cursor-pointer p-3" >
                                ...
                                <tr  class="cursor-pointer manunuzi weight600 ">    
                                    <td class="">${lang('Zilizoongezwa','Added Items')}</td>
                                    <td ></td>
    ...
    if(LoC){
      ...
      let td=  `<table id="SoldItems" class="table table-sm table-bordered table-hover   smallFont" style="width:100%">
        ...
        <tr class="smallFont latoFont">
            ...
            <tr class="smallFont latoFont">
                <th class="manunuzi" colspan=2 >${lang('Zilizoongezwa','Added Items')}</th>
...
$('#SoldItems').DataTable();
    ...
    placedataPanel.prepend(`<h6 id="itmTitle" class="py-3" data-itm=${id} data-val=${val} data-num=1 >${lang('Bidhaa Kuongezwa na Mwenendo kwa','Added Items and trends for')} <i class="darkblue text-capitalize">${itms[0].bidhaaN}</i>  ${lang('Kwa kila Tawi','In each branch')}</h6> ${summary()}`)
...
,
     ...
     showbill = `<div class="d-block ">
                ...
                title = `<h6 class="py-2 " >${lang(`Onesha Notisi za Kuongeza ${MARKED?' Zilizowekwa Alama':''}`,`View ${MARKED?' Marked':''} Adding Notes on`)},<span class="darkblue"> ${dur}</span><h6/>`
             ...
             li+=`<li class="px-2 py-1 border-bottom  ${Number(b.id)==Number(bil_id)?'activeList':''} "  >
                    ...
                    <a class="row px-0 smallerFont text-decoration-none  viewBill2" data-val=${b.id} data-from="${muda.from}" data-to="${muda.to}" style="color: rgb(51, 51, 51);" type="button" >
                        ...
                        <div class="col-7 mx-0 latoFont  pt-0">
                            <span class="text-primary btn btn-default btn-sm smallerFont px-0 latoFont">ADJ-${b.code}</span> | ${moment(b.tarehe).format('DD/MM/YYYY')}
                        </div>
                 ...
                 <tr class="bg-light smallerFont ">
                    ...
                    if(puSize.length>0){
                     ...
                     <tr class="smallerFont latoFont" >
                        <th colspan=2 class="manunuzi" > ${lang('Zilizoongezwa','Added Items')}</th>
                                   ...
                                   //Check weter itmem a s color .......................................//      
                                       ...
                                       if(sz.length>0){
                                            //Check for sizes if tere is more than one size te loop through........
                                               ...
                                               sz.forEach(s => {
                                                 const  wsq = it.wst_S.filter(z=>z.size_id===s.size_id).reduce((a,b)=> a + Number((b.qty)),0),
                                           ...
                                           if(puSize.length>0){
                                             trendsT+=`<td>---</td>`   
                                                     ...
                                                     const  wsq = it.wst_C.filter(z=>z.color_id===c.color_id).reduce((a,b)=> a + Number((b.qty)),0),
                                                      ...
                                                      shq = it.shelf_C.filter(z=>z.id===c.color_id).reduce((a,b)=> a + Number((b.idadi)),0)
                                                          ...
                                                          trendsT+=`<td>${it.kipimo}</td> 
                                                             ...
                                                             <td class="weight600 manunuzi" >${Number(c.qty).toFixed(FIXED_VALUE)}</td> 
                                                             <td class="manunuzi">${floatValue(it.bei*c.qty)}</td> 
                   ...
                   `
                    ...
                    summary += `
                                ...
                                <tr  class="cursor-pointer manunuzi">    
                                    <td class="">${lang('Thamani ya Bidhaa Zilizoongezwa','Total Added Item Worth')}</td>
                                    <td ></td>
       ...
       markbtn = () =>{
          let   markbtn = `<div class="row smallFont my-2 py-2 border-bottom" >
            ...
            <div  class="col-3 text-right">
                <button id="markTheBill" data-from="${bil.from}" data-to="${bil.to}" title="${!bili.mark?lang('Wekea Notisi Alama','Mark Adding Note'):lang('Ondoa Alama','Unmark Adding')}" data-bil=${bili.id} data-mark=${Number(bili.mark)} data-toggle="modal" data-target="#saveBillMarked" class="smallMadebuttons  p-1">
...
const mark = Number($(this).data('mark')),
    ...
    $('#markTite').text(mark?lang('Ondoa Alama kwenye Notisi','unMark Adding'):lang('Wekea  Alama Notisi','Mark Adding'))
    $('#SubmitMark').text(mark?lang('Ondoa Alama','unMark Adding'):lang('Weka  Alama','Mark Adding'))
