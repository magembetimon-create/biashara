    ...
    const ft =d=>moment(d.tarehe).format()>=from && moment(d.tarehe).format() <= to
            ...
            if(dtA.length>0){
               ...
               const   dtS = Adt.dtS.filter(ft),
                 ...
                 theArr(dtS,dtP,dtTr,dtAdj,name,from,to,mark)
                ...
                let fct = getRiportData(dta)
                 ...
                 fct.then(function(data){
                    $("#loadMe").modal('hide');
                     ...
                     theArr(data.mauzo,data.data,data.adjst,data.transfer,name,from,to,mark)
...
function theArr(dtS,dtP,dtAdj,dtTr,name,from,to,mark){
let           
    ...
    if(theData.state.length>0){        
        ...
        let Df = i => !i.online,  Lf = i=>i.online,Na = ByU(),Tawi=branch(),Wvat = withVat()
           ...
           n+=1 
            ...
            let tha = dtP.reduce((a,b)=> a + (Number(b.Bei_kununua)*(b.bqty)/b.uwiano),0),
            ...
            tr+=`
                ...
                <td class=" weight600">${Number(tha).toLocaleString()}</td>
  ...
  const  dt =  theData.state.filter(i=>i.id===val)
       ...
       if(dt.length>0){
            const  theDT = dt[0],
                ...
                wheNoV = v=>v.Na>=0, whenV= v => v.Na===NaV,
                ByVn = NaV==0?wheNoV:whenV,   

                Purchases= {this:theDT.dtP.filter(br).filter(ByVn),other:theDT.dtP.filter(nbr).filter(ByVn)},                    
                ...
                //VENDORS SELECTOR.................//
                vendor = [...new Set(theDT.dtP.filter(br).map(vn=>vn.Na))].map(v=> `<option class="text-capitalize" value="${v}" >${theDT.dtP.filter(vn=>vn.Na===v)[0].Na_name} </option>`) 
                ...
                ItemsTrends({Purchases,transfer,adjst,Sales})
                ...
                $('#toT').val(moment(theDT.To).endOf('day').format())                    
                         ...
                         duraTitle =  theDT.txt   
                         let trnd  = lang('Mwenendo wa Bidhaa Zilizopokelewa','The Trend of Received Items')
                        ...
                        switch (Number(n)) {
                            case 1:
                                placeInvo(Purchases,{transfer,adjst,Sales},{from:theDT.from,to:theDT.To})
                                break;
                            case 2:
                                placeItems({Purchases,transfer,adjst,Sales},val)
                                
                                break;
                            case 3:
                                if(n==3){
                                    placeCategory({Purchases,transfer,adjst,Sales},val)
                                }
                                break;
                            case 4:
                                placeGroups({Purchases,transfer,adjst,Sales},val)
                                break;
...
function ItemsTrends(data){
      ...
      const {Purchases,transfer,adjst,Sales} = data,
            
        tha = {
                this:Purchases.this.reduce((a,b)=> a + (Number(b.Bei_kununua)*(b.bqty)/b.uwiano),0),
                other:Purchases.other.reduce((a,b)=> a + (Number(b.Bei_kununua)*(b.bqty)/b.uwiano),0)
            },
    ...
    //    paid = pay.reduce((a,b)=> a + Number(b.ilolipwa),0),
        ...
        vat={
            ...
            let matawi=$('#Matawini').data('branches')
                   tr = `
                   <tr>
                        <td colspan="${Number(matawi)==0?3:4}" class="brown weight600 manunuzi pb-2 pt-3" >${lang('Kupokelewa','Received')}</td>
                   ...
                   <tr class=" manunuzi weight600" >
                        ...
                        <td class="brown" >${floatValue(tha.this)}</td>
                   ...
                   <tr class="hamishwa forMore" >
                        <td colspan="${Number(matawi)==0?3:4}" class="brown weight600 pb-2 pt-3" >${lang('Bidhaa Kuhamishwa','Items Transfer')} </td>
                   ...
                   if(Mnf.this>0||Mnf.other>0){
                      tr+= `
                        <tr class="production forMore" >
                            <td colspan="${Number(matawi)==0?3:4}" class="brown usage weight600 pb-2 pt-3" >${lang('Uzalishai/Uchakataji Bidhaa','Items Manufacturing/Processing')} </td>
                  ...
                  <tr class="usage forMore" >
                        <td class="brown weight600 pt-3 pb-2" colspan="${Number(matawi)==0?3:4}" >${lang('Matumizi na Upotevu','Usage & Wastage/Damage')} </td>
                  ...
                  <tr class="weight600 usage forMore" >
                   ...
                   <tr class="mauzo forMore"  >
                        <td class="brown weight600 pb-2 pt-3" colspan="${Number(matawi)==0?3:4}" >${lang('Bidhaa Zilizouzwa','Sold Items')} </td>
               ...
               tr+=`<tr class="bg-light weight600 forMore" >
                 ...
                 if(Ongeza.other>0 || Ongeza.this>0){
                        tr+=`<tr class="hamishwa forMore">
                                <td class="brown weight600 pb-2 pt-3" colspan="${Number(matawi)==0?3:4}" >${lang('Bidhaa zilizoongezwa','Added Items')} </td>
                  ...
                  tr+=`<tr class="zilizopo" >
                        <td class="brown weight600 pb-2 pt-3" colspan="${Number(matawi)==0?3:4}"  >${lang('Zilizopo','Onshelf')} </td>
...
let LoC = Number($('#riportChatRist .btn-secondary').data('riport')),
    ...
    dates = [...new Set(ipu.map(i=>moment(i.date).format('YYYY-MM-DD')))].sort(),
    months = [...new Set(dates.map(i=>moment(i).format('YYYYMM')))],

    amounts =  dates.map((d)=>{
        let ft=t=>moment(t.date).format('YYYY-MM-DD') == d,
            pui =  ipu.filter(ft),
            nnt = pui.reduce((a,b)=> a + (Number(b.Bei_kununua)*(b.bqty)/b.uwiano),0),
...
if(LoC){
           ...
           //Draw new Chart................................................................................................................./
           let title = `<h6 class="py-2" >${lang(`Takwimu za mwenendo wa bidhaa zilizopokelewa kwa kila `,`The Trend for Received Goods Statistics on each `)} ${days<=31?lang('Siku','Day'):lang('Mwezi','Month')}, ${duraTitle}<h6/>`
           ...
           var data= {
                   ...
                   datasets: [{
                      ...
                      ,{
                           ...
                           fill: true,
                           lineTension: 0,
                           backgroundColor: "rgba(2, 88, 2, 0.281)",
                      ...
                      ,{
                           ...
                           fill: true,
                           lineTension: 0,
                           backgroundColor: "rgba(0, 128, 0, 0.281)",
...
let title = `<h6 class="py-2" >${lang(`Orodha ya Kupokea Bidhaa kwa kila `,`Receives List on each `)} ${days<=31?lang('Siku','Day'):lang('Mwezi','Month')}, ${duraTitle}<h6/>`
...
const pui = itms.Purchases.this,
    ...
    function theItms(b){
        ...
        return {
            ...
            tha:itm.reduce((a,b)=> a + (Number(b.Bei_kununua)*(b.bqty)/b.uwiano),0),
            idadi:itm.reduce((a,b)=> a + Number((b.bqty)),0),
            bqty:itm.reduce((a,b)=> a + Number((b.bqty)),0),
    ...
    if(LoC){
        ...
        //Draw new Chart................................................................................................................./
        let title = `<h6 class="py-2 " >${lang(`Takwimu ya  Bidhaa Zilizopokelewa na Mwenendo  kwa kila Bidhaa `,`Items Receives and Trends Statistics on each Item  `)}, ${duraTitle}<h6/>`
         $('#theDataPanel').html(`${title} <div class="table-responsive" ><canvas style="${window.outerWidth>800?'max-height:85vh !important':'max-height:70vh !important'} "  id="myChartC"></canvas></div>`);
        ...
        var data= {
                ...
                datasets: [{
                    ...
                    // notice the gap in the data and the spanGaps: true
                    data: bidhaa.map(a=>floatValue(a.tha)),
                    spanGaps: true,
                    }, {
                        label: lang("Matumizi","Usage"),
                        fill: true,
        ...
        <tr class="smallFont latoFont">
            ...
            <tr class="smallFont latoFont">
                <th class="manunuzi" colspan=2 >${lang('Zilizopokelewa','Received')}</th>
...
`   
let title = `<h6 class="py-2" >${lang(`Orodha ya Kupokea na mwenendo kwa kila Bidhaa `,`Receives and Trends List, on each Item Received`)} , ${duraTitle}<h6/>`
$('#theDataPanel').html(title+td)
$('#SoldItems').DataTable();
    }
...
let aina = Categs.state,
    ...
    if(LoC){
        ...
        //Draw new Chart................................................................................................................./
        let title = `<h6 class="py-2 " >${lang(`Takwimu ya Bidhaa Zilizopokelewa namwenendo wa kila Aina ya Bidhaa `,`Items Receives and Trends Statistics on each Item Category  `)}, ${duraTitle}<h6/>`
        ...
        var data= {
            labels: bidhaa.map(b=>b.name),
            datasets: [{
                label:lang("Zilizopokelewa","Received"),
                fill: true,
                lineTension: 0,
                backgroundColor: "#1a8cff",
                borderColor: "#1a8cff", // The main line color
...
`   
let title = `<h6 class="py-2 " >${lang(`Orodha ya Bidhaa Zilizopokelewa na Mwenendo wa kila Aina ya Bidhaa `,`Items Receives and Trend List on each Item Category  `)}, ${duraTitle}<h6/>`
            $('#theDataPanel').html(title+td)
            $('#SoldItems').DataTable();
    }
...
if(LoC){
    ...
    //Draw new Chart................................................................................................................./
    let title = `<h6 class="py-2 " >${lang(`Takwimu ya Bidhaa Zilizopokelewa na mwenendo wa kila Kundi la Bidhaa `,`Items Receives and Trends Statistics on each Item Category Group `)}, ${duraTitle}<h6/>`
    $('#theDataPanel').html(`${title} <div class="table-responsive" ><canvas style="${window.outerWidth>800?'max-height:85vh !important':'max-height:75vh !important'} "  id="myChartC"></canvas></div>`);
...
`   
let title = `<h6 class="py-2 " >${lang(`Orodha ya Zilizopokelewa na Mwenendo wa kila Kundi la Bidhaa `,`Receives and Trend List on each Item Category Group  `)}, ${duraTitle}<h6/>`
        $('#theDataPanel').html(title+td)
        $('#SoldItems').DataTable();
}
...
if(dt.length>0){
     const ftt = i=>  moment(i.tarehe).startOf('day').format() >= moment(muda.from).startOf('day').format()&& moment(i.tarehe).endOf('day').format() <= moment(muda.to).endOf('day').format() ,
      theDT = dt[0],
      vali=Number($('#MauzoAina').val()),online = Number(vali==2 || vali==0),direct = Number(vali==1 || vali == 0),
      Df = i => !i.online,  Lf = i=>i.online,Na = ByU(),Tawi = branch(),NaV = byVendor(),
    ...
    MARKED = theDT.mark  //this varible is a recognition of marked bill list  to  view bill list

    return {
       id:b,
       code:itm.filter(i=>i.bili===b)[0]?.code,
       tarehe:itm.filter(i=>i.bili===b)[0]?.date,
       user:itm.filter(i=>i.bili===b)[0]?.Na_name,
 ...
 let     tr = `<table id="table-Invoices" class="table table-bordered smallFont" style="width:100%">
               <thead>
                    <tr class="smallerFont ">
                        ...
                        <th> ${lang('Kupokea','Receives')}</th>
                        <th> ${lang('Tarehe','Date')}</th>
                        <th> ${lang('Kutoka','From')}</th>
                        <th> ${lang('Thamani','Value')}<span class="text-primary latoFont">(${currencii})</span></th> 
            ...
            billsii.forEach(a=>{
                ...
                tr+= `<tr> 
                  ...
                  <td class="text-capitalize" >${a.user}</td>
            ...
            let d1= moment(muda.from).format('YYYY-MM-DD'),
                ...
                title = `<h6 class="py-2 " >${lang(`Orodha ya notisi za Kupokea Bidhaa ${theDT.mark?' Zilizowekwa alama':''}`,`${theDT.mark?'Marked':''} Receives List on`)},<span class="darkblue"> ${dur}</span><h6/>`
   ...
   matawi = tw.map(t=>formT(t))
    ...
    function formT(t){
        ...
        return {
            ...
            tha:pui.reduce((a,b)=> a + (Number(b.Bei_kununua)*(b.bqty)/b.uwiano),0),
            idadi:pui.reduce((a,b)=> a + Number((b.bqty)),0),
                ...
                <tr class="smallFont text-left" >
                    ...
                    <tbody class="cursor-pointer p-3" >
                                ...
                                <tr  class="cursor-pointer manunuzi weight600 ">    
                                    <td class="">${lang('Zilizopokelewa','Receives')}</td>
                                    <td ></td>
    ...
    if(LoC){
        ...
        var data= {
                ...
                datasets: [{
                    label:lang("Kupokelewa","Receives"),
                    fill: true,
                    lineTension: 0,
                    backgroundColor: "#1a8cff",
                    borderColor: "#1a8cff", // The main line color
      ...
      let td=  `<table id="SoldItems" class="table table-sm table-bordered table-hover   smallFont" style="width:100%">
        ...
        <tr class="smallFont latoFont">
            ...
            <tr class="smallFont latoFont">
                <th class="manunuzi" colspan=2 >${lang('Zilizopokelewa','Receives')}</th>

                <th class="kupokewa" colspan=2 >${lang('Kupokelewa','Reiceved')}</th>
...
$('#SoldItems').DataTable();
    ...
    placedataPanel.prepend(`<h6 id="itmTitle" class="py-3" data-itm=${id} data-val=${val} data-num=1 >${lang('Bidhaa Kupokelewa na Mwenendo kwa','Receives and trends for')} <i class="darkblue text-capitalize">${itms[0].bidhaaN}</i>  ${lang('Kwa kila Tawi','In each branch')}</h6> ${summary()}`)
...
,
     ...
     showbill = `<div class="d-block ">
                ...
                title = `<h6 class="py-2 " >${lang(`Onesha Notisi za kupokea ${MARKED?' Zilizowekwa Alama':''}`,`View ${MARKED?' Marked':''} Receives Notes on`)},<span class="darkblue"> ${dur}</span><h6/>`
...
let bil_id = bil.bil,
    muda = {from:bil.from,to:bil.to}      
       ...
       bilList.state.forEach(b=>{
             li+=`<li class="px-2 py-1 border-bottom  ${Number(b.id)==Number(bil_id)?'activeList':''} "  >
                    ...
                    <a class="row px-0 smallerFont text-decoration-none  viewBill2" data-val=${b.id} data-from="${muda.from}" data-to="${muda.to}" style="color: rgb(51, 51, 51);" type="button" >
                        <div class="col-6 mx-0">
                             <strong class="text-capitalize" >${b.user}</strong> 
                        </div>
                        ...
                        <div class="col-7 mx-0 latoFont  pt-0">
                            <span class="text-primary btn btn-default btn-sm smallerFont px-0 latoFont">RC-${b.code}</span> | ${moment(b.tarehe).format('DD/MM/YYYY')}
                        </div>

                        <div class="col-5 mx-0 text-right pt-1">

                                <span class="py-1 px-0 btn btn-sm  btn-default smallFont brown weight400  latoFont"  >
                                    <span class="smallFont text-primary" >${currencii}</span>. ${floatValue(b.shelf)}
                                </span>
 ...
 const    bili=billData.state.find(b=>b.id === bil.bil)
     ...
     if(bili){
        ...
        const  {itms_C:puColor,itms_S:puSize,adjst_C,adjst_S,stock_C,stock_S,id,sale_S,sale_C} = bili,
             {allpu,pui,sold,adj,tha,upotevu,uzalishaji,sale,matumizi,shelf,saleV}  = bilList.state.find(b=>b.id==id),
      ...
      //  console.log({stock_C,stock_S,id,sale_S,sale_C})    
                 ...
                 <tr class="bg-light smallerFont ">
                    ...
                    if(puSize.length>0){
                     ...
                     <tr class="smallerFont latoFont" >
                        <th colspan=2 class="manunuzi" > ${lang('Zilizopokelewa','Received')}</th>
                                  ...
                                  if(Brnch){
                                   ...
                                   //Check weter itmem a s color .......................................//      
                                       ...
                                       if(cc>0){ 
                                           ...
                                           it.puC.forEach(c=>{
                                               let sz = it.puS.filter(cr=>cr.color_id==c.color_id),
                                                   szl = sz.length,
                                                   szll=szl+1,
                                       ...
                                       if(sz.length>0){
                                            //Check for sizes if tere is more than one size te loop through........
                                               ...
                                               sz.forEach(s => {
                                                 let  wsq = it.wst_S.filter(z=>z.size_id==s.id).reduce((a,b)=> a + Number((b.qty)),0),
                                                      mfq = it.mfg_S.filter(z=>z.size_id==s.id).reduce((a,b)=> a + Number((b.qty)),0),
                                                      usq = it.usd_S.filter(z=>z.size_id==s.id).reduce((a,b)=> a + Number((b.qty)),0),
                                                      slq = it.saS.filter(z=>z.size_id==s.id).reduce((a,b)=> a + Number((b.idadi-b.returned)),0),
                                                      shq = it.shelf_S.filter(z=>z.id==s.id).reduce((a,b)=> a + Number((b.idadi)),0)
                                           ...
                                           if(puSize.length>0){
                                             trendsT+=`<td>---</td>`   
                                                     ...
                                                     let  wsq = it.wst_C.filter(z=>z.color_id==c.id).reduce((a,b)=> a + Number((b.qty)),0),
                                                      mfq = it.mfg_C.filter(z=>z.color_id==c.id).reduce((a,b)=> a + Number((b.qty)),0),
                                                      usq = it.usd_C.filter(z=>z.color_id==c.id).reduce((a,b)=> a + Number((b.qty)),0),
                                                      slq = it.saC.filter(z=>z.color_id==c.id).reduce((a,b)=> a + Number((b.idadi-b.returned)),0),
                                                      shq = it.shelf_C.filter(z=>z.id==c.id).reduce((a,b)=> a + Number((b.idadi)),0)
                   ...
                   `
                    ...
                    summary += `
                                ...
                                <tr  class="cursor-pointer manunuzi">    
                                    <td class="">${lang('Thamani ya Zilizopokelewa Jumla','Total Received Worth')}</td>
                                    <td ></td>
       ...
       markbtn = () =>{
          let   markbtn = `<div class="row smallFont my-2 py-2 border-bottom" >
            ...
            <div  class="col-3 text-right">
                <button id="markTheBill" data-from="${bil.from}" data-to="${bil.to}" title="${!bili.mark?lang('Wekea Notisi Alama','Mark Receive Note'):lang('Ondoa Alama','Unmark Receive')}" data-bil=${bili.id} data-mark=${Number(bili.mark)} data-toggle="modal" data-target="#saveBillMarked" class="smallMadebuttons  p-1">
...
const mark = Number($(this).data('mark')),
    ...
    $('#markTite').text(mark?lang('Ondoa Alama kwenye Notisi','unMark Receive'):lang('Wekea  Alama Notisi','Mark Receive'))
    $('#SubmitMark').text(mark?lang('Ondoa Alama','unMark Receive'):lang('Weka  Alama','Mark Receive'))
