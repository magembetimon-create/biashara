function placedataTotable(data){
 ...
 <thead>
     <tr class="smallFont ">
         ...
         <th> ${lang('Thamani kwa Mauzo','Value for Sales')}</th>
         <th>Action</th>
     </tr>
 </thead>
 ...
 let idadi_item=0,zilizopo=0,thamani=0,mauzo_tegemeo=0,num=0
     if(sambazia.state.length>0){ 
         ...
         num+=1
             ...
             <td class="text-capitalize" >${itm.addres}</a></td>
             <td>+${itm.code} ${Number(itm.simu1)} </td>
             <td> `
             if(itm.simu2){
                 trp+= `+${itm.code} ${Number(itm.simu2)}`
             }else{
                 trp+=lang('Hakuna','Null')
             }
             
             trp+=` </td>
                <td class="font-weight-bold" > <a class="btn btn-default text-primary" href="/stoku/bidhaaReg?sup=${itm.id}" > ${itm.zote}</a></td>
                <td> <span class="font-weight-bold">${Number(itm.idadi)}</span> </td>
                <td class="maroon">${Number(itm.thamani).toLocaleString()}</td>
                <td class="greyscale">${Number(itm.thamaniM).toLocaleString()}</td>  
           
             <td >
             <div class="d-flex">
 
                 <button data-target="#wasambazaji-modal" data-toggle="modal" onclick="$('#form-wasambazaji').data({edit:1,value:${itm.id}});$('#sambazaji-jina').val('${itm.jina}');$('#sambazaji-sehemu').val('${itm.addres}');$('.sambCode').val(${itm.code});$('#sambazaji-simu').val('${itm.simu1}');$('#sambazaji-simu2').val('${itm.simu2}');$('#sambazaji-mail').val('${itm.email ||'' }')"  data-id=${itm.id} class="btn btn-default btn-sm">
                  ...
                  onclick="
                       if(${itm.zote==0}){
                           if(confirm('${lang('Odoa Aina ya bidhaa','Remove Items Cateory')}?')){
                               let data={data:{val:${itm.id},csrfmiddlewaretoken:$('input[name=csrfmiddlewaretoken]').val()},url:'/stoku/futaSup'};
                                 saveStokuData(data) 
                           }
                          
                       }else{
                           alert('${lang('Msambazaji hawezi kufutwa kutokana na Kuwa na Bidhaa','Supplier can not be deleted due to presence number of associated brand items')}')
                         }
