...
<thead>
    <tr class="smallFont ">
        ...
        <th> ${lang('Thamani kwa Kuuza','Value for Sales')}</th>
        <th>Action</th>
    </tr>
</thead>
...
let idadi_item=0,zilizopo=0,thamani=0,mauzo_tegemeo=0,num=0
    ...
    if(data.products.length>0){ 
        ...
        trp+=`   </td> 
            ...
            <td>${itm.zote}</td>
            <td>${itm.aina_id ? `<a href="/stoku/bidhaaReg?f=${itm.aina_id}">${tbSafeText(itm.aina, lang('Bila Aina', 'Uncategorized'))}</a>` : `<span>${tbSafeText(itm.aina, lang('Bila Aina', 'Uncategorized'))}</span>`}</td>
            <td><a  href="/stoku/bidhaaReg?bf=${itm.kampuni_id}"> ${tbSafeText(itm.kampuni)}</a></td>`
            if(Number(Number(itm.idadi)/Number(itm.idadi_jum))>1){

                trp+=`  <td><span class="smallerFont text-primary">${itm.vipimo_jum} </span></td>
                        <td> <span class="font-weight-bold">${Number(Number(itm.idadi)/Number(itm.idadi_jum)).toFixed(FIXED_VALUE)}</span> </td> 
                        <td class="brown weight600">${Number(Coast.toFixed(FIXED_VALUE)).toLocaleString()}</td>
                        <td class="brown weight600">${Number(Number(itm.thamaniM).toFixed(FIXED_VALUE)).toLocaleString()}</td>`
        ...
        trp+=` 
            <td >
            <div class="d-flex">
                <a href="/stoku/bidhaapanel?f=${itm.id}" class="btn btn-default  btn-sm text-primary " title="${lang('Onesha','View')}"> 
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-eye" viewBox="0 0 16 16">
                        <path d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8zM1.173 8a13.133 13.133 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5c2.12 0 3.879 1.168 5.168 2.457A13.133 13.133 0 0 1 14.828 8c-.058.087-.122.183-.195.288-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5c-2.12 0-3.879-1.168-5.168-2.457A13.134 13.134 0 0 1 1.172 8z"/>
                        <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5zM4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0z"/>
                    </svg>
                </a>
                <button type="button" data-stock="${itm.stock_id}" class="btn btn-default btn-sm text-primary open-item-edit" title="${lang('Kuediti','Edit')}">
